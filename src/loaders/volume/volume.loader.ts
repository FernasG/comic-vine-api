import { Volume, Connection } from "@database";
...
export const VolumeLoader = (async () => {
  ...
  while (true) {
    ...
    for (const vol of volumes) {
      const volume = await connection.getRepository(Volume).findOneBy({ id: vol.id });

      if (volume) continue;

      const insertResult = await connection.getRepository(Volume).insert(vol);
